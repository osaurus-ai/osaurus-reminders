...
struct GetRemindersTool: Tool {
    ...
    func run(args: String) -> String {
        ...
        if status == "incomplete" {
            predicate = store.predicateForIncompleteReminders(withDueDateStarting: start, ending: end, calendars: calendars)
            ...
            predicate = store.predicateForCompletedReminders(withCompletionDateStarting: start, ending: end, calendars: calendars)
        } else {
            ...
            let p1 = store.predicateForIncompleteReminders(withDueDateStarting: start, ending: end, calendars: calendars)
            let p2 = store.predicateForCompletedReminders(withCompletionDateStarting: start, ending: end, calendars: calendars)
            ...
            guard let json = try? JSONEncoder().encode(dtos) else { return "{\"error\": \"Encoding failed\"}" }
...
struct SearchRemindersTool: Tool {
    ...
    func run(args: String) -> String {
        ...
        let p2 = store.predicateForCompletedReminders(withCompletionDateStarting: oneMonthAgo, ending: nil, calendars: nil)
...
struct CreateReminderTool: Tool {
    ...
    func run(args: String) -> String {
        ...
        // Handle List
        if let listName = input.listName {
            if let list = ReminderManager.shared.getLists().first(where: { $0.title.caseInsensitiveCompare(listName) == .orderedSame }) {
        ...
        do {
            ...
            return "{\"success\": true, \"id\": \"\(reminder.calendarItemIdentifier)\", \"message\": \"Reminder created\"}"
        } catch {
